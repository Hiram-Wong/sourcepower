<template>
  <div class="detail view-container">
    <div class="content">
      ...
      <t-alert
        v-if="detailData.sensitive"
        theme="error"
        message="当前敏感数据, 注意周围环境并调低音量!"
        close
        class="alert-item"
      />
      ...
      <t-card title="数据内容" :bordered="false" class="card-item t-card-tag detail-content">
        ...
        >
          <div v-if="detailData.data" class="detail-content-item">
            ...
            <div>
              <t-link
                v-if="formatUrl(detailData.data)"
                :href="detailData.data"
                target="_blank"
      ...
      <t-card title="探讨交流" :bordered="false" class="card-item t-card-tag detail-comments">
        ...
        <template v-for="item in commentData" :key="item.comment_id">
          <t-comment
            :avatar="formatAvatar(item.user.email)"
          ...
          >
            ...
            <template #actions>
              ...
              <t-space
                key="delete"
                :size="6"
                v-if="item.user.id === userStore.userInfo.id"
            ...
            <template #reply>
              <template v-for="reply in item.reply" v-if="item.reply_count !== 0">
                <t-comment :author="reply.user.username" :datetime="formatDate(reply.created_at)">
                  ...
                  <template #content>
                    <render-markdown v-model:markdownText="reply.compiled_content" />
                  </template>
                  <template #actions>
              ...
              <load-view
                :current="item.reply.length"
                :total="item.reply_count"
                @load="onLoadMoreReply(item.comment_id, item.reply.length)"
...
const onShareZyplayer = async () => {
  try {
    ...
    if (detailData.value.type === 'site') {
      ...
      const formatData = detailData.value.data.startsWith('http')
        ? detailData.value.data
        : new URL(`${BASE_API_URL}${detailData.value.data}`).href;
      const data = {

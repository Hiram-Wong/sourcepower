...
const cheerio = require("./utils/cheerio.min");
const gbkTool = require("./utils/gbk");
const NODERSA = require("./utils/node-rsa");
  ...
  if (js === "maomi_aes.js") {
    ...
    const decrypt = (word) => {
      ...
      const decrypted = CryptoJS.AES.decrypt(
        CryptoJS.enc.Base64.stringify(word),
        key,
        {
          iv: iv,
...
const maoss = (jxurl, ref, key) => {
  ...
  try {
    ...
    let temp = { getVideoInfo }; // 防止tree-shake
 ...
 */
const fixAdM3u8 = (m3u8_text, m3u8_url, ad_remove) => {
  if (
    (!m3u8_text && !m3u8_url) ||
    (!m3u8_text && m3u8_url && !m3u8_url.startsWith("http"))
  ) {
    return "";
  }
  if (!m3u8_text) {
  ...
  let m3u8_body = m3u8_text
    .slice(m3u8_text.indexOf("#EXTINF"), m3u8_text.indexOf("#EXT-X-ENDLIST"))
    .trim();
  ...
  for (let murl of murls) {
    if (ad_remove && new RegExp(ad_remove).test(murl)) {
      ...
      if (
        !murl_list[murl_list.length - 1].startsWith("http") &&
        m3u8_url.startsWith("http")
      ) {
        murl_list[murl_list.length - 1] = urljoin(
          m3u8_url,
          murl_list[murl_list.length - 1]
        );
      }
 ...
 */
const fixAdM3u8Ai = (m3u8_url, headers = null) => {
...
const verifyCode = (url) => {
  ...
  while (cnt < OCR_RETRY) {
    try {
      ...
      if (!cookie) {
        let setCk = Object.keys(json).find(
          (it) => it.toLowerCase() === "set-cookie"
        );
 ...
 */
const request = (url, obj = undefined, ocr_flag = false) => {
  ...
  if (obj?.headers) {
    ...
    for (const [customHeader, originalHeader] of Object.entries(
      customHeaders
    )) {
  ...
  console.log(
    `[t3][request]url:${url}| method:${obj.method || "GET"}| body:${
      obj.body || ""
    }`
  );
 ...
 */
const reqCookie = (url, obj = {}, all_cookie = false) => {
...
const homeParse = (homeObj) => {
            ...
            if (list && list.length > 0) {
              list.forEach((it, idex) => {
                try {
                  ...
                  if (
                    homeObj.cate_exclude &&
                    new RegExp(homeObj.cate_exclude).test(name)
                  ) {
                    return;
                  }
                  let url = _pd(it, p[2]);
  ...
  classes = classes.filter(
    (it) =>
      !homeObj.cate_exclude ||
      !new RegExp(homeObj.cate_exclude).test(it["type_name"])
  );
...
const homeVodParse = (homeVodObj) => {
          ...
          for (let item2 of items2) {
            try {
              ...
              for (let _p5 of p5.split("+")) {
                let link = !homeVodObj.detailUrl
                  ? _pd(item2, _p5, MY_URL)
                  : _pdfh(item2, _p5);
                links.push(link);
              }
  ...
  if (rule["图片来源"]) {
    d.filter((it) => it["vod_pic"] && it["vod_pic"].startsWith("http")).forEach(
      (it) => {
...
const categoryParse = (cateObj) => {
  ...
  if (rule["filter_url"]) {
    if (!/fyfilter/.test(url)) {
      url +=
        !url.endsWith("&") && !rule["filter_url"].startsWith("&") ? "&" : "";
      url += rule["filter_url"];
    } else {
    ...
    if (rule["filter_def"] && typeof rule["filter_def"] === "object") {
      try {
        if (
          Object.keys(rule["filter_def"]).length > 0 &&
          rule["filter_def"].hasOwnProperty(cateObj.tid)
        ) {
    ...
    try {
      ...
      if (html) {
        ...
        if (!html.includes(searchObj.wd)) {
          console.log(
            `[t3][search]疑似搜索失败, 结果未包含关键字, 源数据:${html}`
          );
        }
...
const vodDeal = (vod) => {
  ...
  if (rule["tab_remove"]?.length > 0 || rule["tab_order"]?.length > 0) {
    ...
    if (rule["tab_remove"]?.length > 0) {
      tab_removed_list = vod_play_from.filter(
        (it) => !rule["tab_remove"].includes(it)
      );
  ...
  if (
    rule["tab_rename"] &&
    typeof rule["tab_rename"] === "object" &&
    Object.keys(rule["tab_rename"]).length > 0
  ) {
...
const tellIsJx = (url) => {
  try {
    const is_vip =
      !/\.(m3u8|mp4|m4a)$/.test(url.split("?")[0]) && isGenuine(url);
    return is_vip ? 1 : 0;
  } catch (e) {
...
const proxyParse = (proxyObj) => {
  ...
  if (proxyObj.proxy_rule) {
    console.log(
      `[t3][proxy][proxyParse]准备执行本地代理规则:${proxyObj.proxy_rule}`
    );
 ...
 * @returns {string}
 */
const home = (filter = {}, home_html = "", class_parse = "") => {
  filter = filter || {};
  home_html = home_html || "";
...
const search = (wd, quick, pg) => {
  let search_encoding =
    rule["search_encoding"]?.toLowerCase() || rule["encoding"]?.toLowerCase();
